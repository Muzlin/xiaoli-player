...
class _PayPageState extends State<PayPage> {
  ...
  Future<void> _scan() async {
    // 让用户选：从图片识别(稳)或摄像头(实验)。默认图片识别。
    final mode = await showModalBottomSheet<String>(
      context: context,
      builder: (c) => SafeArea(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ListTile(
              leading: const Icon(Icons.image_outlined, color: Color(0xFF07C160)),
              title: const Text('从图片识别二维码'),
              subtitle: const Text('选一张含二维码的图片（推荐，最稳）'),
              onTap: () => Navigator.pop(c, 'image')),
          ListTile(
              leading: const Icon(Icons.photo_camera_outlined),
              title: const Text('摄像头扫描'),
              subtitle: const Text('对准实物二维码（实验）'),
    ...
    if (mode == null) return;
    final r =
        mode == 'image' ? await NativeQr.scanImage() : await NativeQr.scan();
    if (!mounted) return;
    if (r == null) {
      _snack(mode == 'image' ? '图片里没识别到二维码' : '没扫到');
      return;
    }
